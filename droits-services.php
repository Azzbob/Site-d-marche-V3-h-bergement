...
$stmt = $pdo->prepare(
    "SELECT l.*, c.nom AS cat_nom
     ...
     WHERE l.actif = 1 AND c.nom = 'Droits & Services'
...
if ($user_connecte && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_favori'])) {
    ...
    header('Location: droits-services.php');
    exit;
}
...
<div class="page-header">
  <h1 class="page-header__title">Droits &amp; Services</h1>
  <p class="page-header__sub">Toutes vos démarches liées aux droits et services en un seul endroit</p>
</div>
      ...
      <div class="lien-card">
        ...
        <div class="lien-card__body">
          <div class="lien-card__left">
            <?php if ($lien['logo']): ?>
              <img src="<?= htmlspecialchars($lien['logo']) ?>" alt="<?= htmlspecialchars($lien['titre']) ?>" class="lien-card__logo">
            <?php else: ?>
              <div class="lien-card__logo-placeholder">LOGO</div>
            <?php endif; ?>
            <a href="<?= htmlspecialchars($lien['url']) ?>" target="_blank" rel="noopener noreferrer" class="lien-card__url">
          ...
          <?php if ($user_connecte): ?>
            <form method="POST" style="margin:0">
              ...
              <button type="submit" name="toggle_favori" class="lien-card__fav <?= $estFavori ? 'lien-card__fav--active' : '' ?>">
                <span class="lien-card__fav-icon"><?= $estFavori ? '★' : '☆' ?></span>
                <?= $estFavori ? 'dans vos favoris' : 'ajouter au favoris' ?>
              </button>
            </form>
          <?php else: ?>
            <a href="connexion.php" class="lien-card__fav">
