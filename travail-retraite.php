<?php
// ============================================================
//  travail-retraite.php  —  Page catégorie Travail & Retraite
// ============================================================
session_start();
require_once 'db.php';
...
// Récupère tous les liens de la catégorie "Travail & Retraite"
$stmt = $pdo->prepare(
    "SELECT l.*, c.nom AS cat_nom
     FROM liens l
     LEFT JOIN categories c ON c.id = l.categorie_id
     WHERE l.actif = 1 AND c.nom = 'Travail & Retraite'
...
if ($user_connecte && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_favori'])) {
    ...
    header('Location: travail-retraite.php');
    exit;
}
...
<div class="page-header">
  <h1 class="page-header__title">Travail &amp; Retraite</h1>
  <p class="page-header__sub">Toutes vos démarches liées au travail et à la retraite en un seul endroit</p>
</div>
